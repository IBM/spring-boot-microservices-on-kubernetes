...
public class TriggerEmail {
	...
	private String sendEmail(@RequestBody Map<String, Object> payload) {
		...
		try {
			if (email_openwhisk_url.isEmpty()) {
				...
				headers.add("Accept", "*/*");
				String json = "{\"text\": \"" + slack_message + ", " + payload.get("balance") + "\",\"sender\": \"" + sender + "\",\"receiver\": \"" + receiver + "\",\"password\": \"" + password + "\",\"subject\": \"Office-Space Notification\"}";
	...
	private String sendSlack(@RequestBody Map<String, Object> payload) {
		try {
			if (!slack_url.isEmpty()) {
				...
				headers.add("Content-Type", "application/json");
				headers.add("Accept", "*/*");
				String json = "{\"text\": \"" + slack_message + ", " + payload.get("balance") + "\"}";
	...
	private String trigger() {
		RestTemplate rest = new RestTemplate();
		HttpHeaders headers = new HttpHeaders();
		String server = "http://localhost:8080/slack";
		headers.add("Content-Type", "application/json");
	  headers.add("Accept", "*/*");
		String json = "{}";
